...
class PoseEngine {
  constructor(videoElement, canvasElement, options = {}) {
    ...
    this.onGhostUpdate = options.onGhostUpdate || (() => {});
    ...
    // Tempo & Time Under Tension (TUT) Tracking
    this.repStartTime = 0;
    this.inflectionTime = 0;

    // Ghost Rival Mode (التنافس ضد الشبح)
    this.ghostModeEnabled = false;
    this.ghostReps = 0;
    this.ghostTargetReps = 12;
    this.ghostPaceSeconds = 3.0;
  ...
  resetCounter() {
    ...
    if (this.ghostModeEnabled) {
      ...
      this.onGhostUpdate({
        ghostReps: 0,
        targetReps: this.ghostTargetReps,
        userReps: 0,
        lead: 0
      });
    }
  ...
  // Record beginning of eccentric / concentric movement
  triggerRepStart() {
    if (!this.repStartTime) {
      this.repStartTime = performance.now();
    }
  }

  // Complete repetition with Tempo & TUT (Time Under Tension) evaluation
  triggerRepComplete(baseQuality = 'good') {
    this.repCount++;
    this.cuePlayed = false;
    ...
    if (this.repStartTime > 0) {
      tempoTime = Math.round(((now - this.repStartTime) / 1000) * 10) / 10;
    } else {
      tempoTime = 2.4;
    }

    // Time Under Tension >= 2.4s denotes controlled cadence (Super Rep)
    const isSuperRep = tempoTime >= 2.4;
    this.repStartTime = 0;
    this.inflectionTime = 0;

    // Rep event with quality, tempo in seconds, and super rep flag
    this.onRep(this.repCount, isSuperRep ? 'super' : baseQuality, tempoTime, isSuperRep);

    // If Ghost Rival Mode is active, push live race telemetry
    if (this.ghostModeEnabled) {
      this.onGhostUpdate({
        ghostReps: this.ghostReps,
        targetReps: this.ghostTargetReps,
        userReps: this.repCount,
        lead: this.repCount - this.ghostReps
      });
    }
  }

  // Ghost Rival Mode: Virtual Pacer or Previous Record
  startGhostRival(targetReps = 10, paceSeconds = 3.2) {
    this.stopGhostRival();
    this.ghostModeEnabled = true;
    this.ghostTargetReps = targetReps;
    this.ghostPaceSeconds = paceSeconds;
    this.ghostReps = 0;
    ...
    this.ghostTimer = setInterval(() => {
      if (!this.isRunning || !this.ghostModeEnabled) return;
      if (this.ghostReps < this.ghostTargetReps) {
        this.ghostReps++;
        this.onGhostUpdate({
          ghostReps: this.ghostReps,
          targetReps: this.ghostTargetReps,
  ...
  evaluateExercise(lm) {
    ...
    switch (this.currentExercise) {
      ...
      case 'squats': {
        ...
        if (progress > 15) this.triggerRepStart();

        if (angle < 95) {
          if (this.state === 'UP') {
            this.state = 'DOWN';
            this.inflectionTime = performance.now();
            if (!this.cuePlayed) { this.onCue(); this.cuePlayed = true; }
          }
        } else if (angle > 160) {
          if (this.state === 'DOWN') {
            this.state = 'UP';
            this.triggerRepComplete(progress >= 90 ? 'perfect' : 'good');
          }
        }
        break;
      }
      ...
      case 'lunges': {
        ...
        if (angle < 100) {
          ...
          if (this.state === 'DOWN') {
            this.state = 'UP';
            this.triggerRepComplete('perfect');
          }
        }
        break;
      }
          ...
          if (!this.plankStartTime) {
            ...
            if (elapsed > this.repCount) {
              ...
              if (this.repCount % 5 === 0) {
                this.onRep(this.repCount, 'perfect', 5.0, true);
              } else {
          ...
          if (cfg) {
            ...
            if (startA > endA) {
              ...
              if (angle <= endA + 8) {
                ...
                if (this.state === 'DOWN') {
                  this.state = 'UP';
                  this.triggerRepComplete(progress >= 85 ? 'perfect' : 'good');
                }
              }
            } else {
